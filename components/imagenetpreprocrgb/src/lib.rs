...
impl Guest for ImagenetPreProcessor {
    fn convert(
        tensor: Tensor, 
        _to_shape: Option<Dimensions>, 
        _to_dtype: Option<DataType>
        ...
        let converted_tensor_data = match preprocess(&tensor_data, HEIGHT, WIDTH) {
            Ok(t) => t,
            Err(e) => {
                return Err(MlError::Internal(e.to_string()));
            }
        };
